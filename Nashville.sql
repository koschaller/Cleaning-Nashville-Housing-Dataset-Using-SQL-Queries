...
SELECT SUBSTRING(propertyaddress
                 FOR POSITION(',' IN propertyaddress)-1) AS prop_address,
       SUBSTRING(propertyaddress
                 FROM POSITION(',' IN propertyaddress)+ 1) AS prop_city
FROM nashville;


ALTER TABLE nashville 
	ADD COLUMN prop_address VARCHAR(255),
        ADD COLUMN prop_city VARCHAR(50);


UPDATE nashville
SET prop_address = SUBSTRING(propertyaddress FOR POSITION(',' IN propertyaddress)-1);

UPDATE nashville
SET prop_city = SUBSTRING(propertyaddress FROM POSITION(',' IN propertyaddress)+ 1);
...
SELECT SPLIT_PART(owneraddress, ',', 1) AS own_address,
       SPLIT_PART(owneraddress, ',', 2) AS own_city,
       SPLIT_PART(owneraddress, ',', 3) AS own_state
FROM nashville
	
ALTER TABLE nashville 
	ADD COLUMN own_address VARCHAR(255),
        ADD COLUMN own_city VARCHAR(50),
        ADD COLUMN own_state VARCHAR(5);

UPDATE nashville
SET own_address = SPLIT_PART(owneraddress, ',', 1);

UPDATE nashville
SET own_city = SPLIT_PART(owneraddress, ',', 2);

UPDATE nashville
SET own_state = SPLIT_PART(owneraddress, ',', 3);
